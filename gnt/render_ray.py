...
def render_rays_hybrid(
    ...
    rgb_feat_clean, ray_diff_clean, mask_clean = projector.compute(
        pts,
        ray_batch["camera"],
        ray_batch['src_rgbs'] if src_ray_batch is None else src_ray_batch['src_rgbs'],
        ray_batch['src_cameras'] if src_ray_batch is None else src_ray_batch['src_cameras'],
        featmaps=featmaps_clean[0],
    )  # [N_rays, N_samples, N_views, x]
    rgb_clean = model.net_coarse(rgb_feat_clean, ray_diff_clean, mask_clean, pts, ray_d)
    
    if ret_alpha:
        rgb_clean, weights_clean = rgb_clean[:, 0:3], rgb_clean[:, 3:]
        depth_map_clean = torch.sum(weights_clean * z_vals, dim=-1)
    else:
        weights_clean = None
        depth_map_clean = None

    ret["outputs_coarse"] = {
        "rgb": rgb_clean if args.use_clean_color else rgb, 
        "weights": weights_clean if args.use_clean_density else weights, 
        "depth": depth_map
        }
